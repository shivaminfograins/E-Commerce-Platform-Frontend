...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TablePagination, TableSortLabel, IconButton, Box, Typography } from "@mui/material";
...
function CategoryTable({ categories = [], onEdit, onDelete }) {
  ...
  const filteredSortedCategories = stableSort(categories, getComparator(order, orderBy));
  const paginatedCategories = filteredSortedCategories.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
              ...
              <TableRow key={category.id} hover sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                ...
                <TableCell>
                  <Box
                    sx={{
                      ...
                      bgcolor: category.status === "Active" ? "#d1fae5" : "#f1f5f9",
                      color: category.status === "Active" ? "#065f46" : "#64748b"
                    }}
                  >
                    {category.status}
                  </Box>
                ...
                <TableCell align="right">
                  <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}>
                    <IconButton onClick={() => onEdit(category)} sx={{ color: "#3b82f6", p: 1, bgcolor: "rgba(59, 130, 246, 0.05)", "&:hover": { bgcolor: "rgba(59, 130, 246, 0.1)" } }}>
                    ...
                    <IconButton onClick={() => onDelete(category)} sx={{ color: "#ef4444", p: 1, bgcolor: "rgba(239, 68, 68, 0.05)", "&:hover": { bgcolor: "rgba(239, 68, 68, 0.1)" } }}>
