...
function CategoryImageTable({ categoryImages = [], categories = [], onEdit, onDelete }) {
  ...
  const getCategoryName = (catId) => {
    ...
    return cat ? cat.name : `Category (ID: ${catId})`;
  };
  ...
  const descendingComparator = (a, b, orderBy) => {
    ...
    // If sorting by category name, resolve it first
    if (orderBy === "category") {
  ...
  const filteredSortedImages = stableSort(categoryImages, getComparator(order, orderBy));
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden", borderRadius: "16px", border: "1px solid rgba(15, 23, 42, 0.05)", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.02)" }}>
      <TableContainer>
        <Table sx={{ minWidth: 650 }} aria-label="category image table">
          <TableHead>
              ...
              <TableRow key={row.id} hover sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                ...
                <TableCell>
                  <Avatar
                    ...
                    alt={row.alt_text || "Category Showcase"}
                    variant="rounded"
                    sx={{
                      width: 60,
                      height: 60,
